...
class fileReader {//CLASE FILEREADER.
	...
	public:
   		fileReader( const char * fileName,double act, int strate, int llave);//CONSTRUCTOR.
		...
		bool personal(int woker);//MÉTODO PERSONAL PARA ESTRATEGIA PARES.
	...
	private:
		...
		int vectInicio[30];//POSICIÓN DE INICIO DE LOS HILOS.
		int vectActual[30];//POSICIÓN ACTUAL DE LOS HILOS.
		int vectCant[30];//CANTIDAD DE EJECUCIONES.
		int divisor; 
};
#endif
